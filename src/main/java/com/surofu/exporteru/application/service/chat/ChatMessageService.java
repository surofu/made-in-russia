...
public class ChatMessageService {
  ...
  public ChatMessageDTO sendMessage(SendMessageRequest request, Long senderId) {
    ...
    if (request.getChatId() != null) {
      ...
      chat = chatRepository.findById(chatDTO.getId())
          .orElseThrow(
              () -> new EntityNotFoundException("Chat not found with id: " + chatDTO.getId()));
    } else {
    ...
    if (request.getAttachments() != null && !request.getAttachments().isEmpty()) {
      List<MessageAttachment> attachments =
          attachmentService.saveAttachments(savedMessage, request.getAttachments());
  ...
  public MarkAsReadResult markAsReadAndGetBroadcastInfo(Long messageId, Long userId) {
    ...
    log.debug("Sent read notification to sender {} for message {}", message.getSender().getId(),
        messageId);
   ...
   */
  public record MarkAsReadResult(Long chatId, boolean isAdmin) {
  }
