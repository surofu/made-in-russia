...
public class SupportApplicationService implements SupportService {
    ...
    public SendSupportMail.Result sendSupportMail(SendSupportMail operation) {
        ...
        try {
            subject = "Помощь - " + translationRepository.translateToRu(operation.getSubject()).getTranslations()[0].getText();
            body = translationRepository.translateToRu(operation.getBody()).getTranslations()[0].getText();
        } catch (Exception e) {
