...
public class AuthApplicationService implements AuthService {
    ...
    public Register.Result register(Register operation) {
        ...
        String rawUserLogin = operation.getUserLogin().toString();
        ...
        } else if (Locale.forLanguageTag("zh").equals(operation.getLocale())) {
            userLogin.setTransliteration(new HstoreTranslationDto(
                    transliterationLogin,
                    transliterationLogin,
                    rawUserLogin
    ...
    public VerifyEmail.Result verifyEmail(VerifyEmail operation) {
        ...
        try {
            verificationCodeFromCache = userVerificationRedisCacheManager.getVerificationCode(operation.getUserEmail());
        ...
        if (user.getVendorDetails() != null) {
            ...
            if (vendorDetails.getAddress() != null &&
                ...
                try {
                    HstoreTranslationDto addressTranslations = translationRepository
                            .expand(vendorDetails.getAddress().toString());
                    vendorDetails.getAddress().setTranslations(addressTranslations);
                } catch (IOException e) {
                    TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
    ...
    public ForceRegister.Result forceRegister(ForceRegister operation) {
        ...
        String rawUserLogin = operation.getLogin().toString();
        UserLogin userLogin = UserLogin.of(rawUserLogin);
        String transliterationLogin = TransliterationManager.transliterate(rawUserLogin);
