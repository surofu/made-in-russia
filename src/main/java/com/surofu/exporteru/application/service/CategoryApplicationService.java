...
public class CategoryApplicationService implements CategoryService {
    ...
    public UpdateCategoryById.Result updateCategoryById(UpdateCategoryById operation) {
        ...
        existingOkvedCategories.removeIf(okved -> !newOkvedIds.contains(okved.getOkvedId()));

        Set<String> existingIds = existingOkvedCategories.stream()
                .map(OkvedCategory::getOkvedId)
                .collect(Collectors.toSet());

        for (String okvedId : newOkvedIds) {
            if (!existingIds.contains(okvedId)) {
                OkvedCategory newOkvedCategory = new OkvedCategory();
                newOkvedCategory.setCategory(category);
                newOkvedCategory.setOkvedId(okvedId);
                existingOkvedCategories.add(newOkvedCategory);
            }
        }
