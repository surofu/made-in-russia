...
import static com.surofu.exporteru.application.components.telegrambot.TelegramBotUtils.*;
...
public class TelegramBotRegisterHandler {
    ...
    private Void stepAccountTypeRequest(Update update) {
        ...
        String requestText = localizationManager.localize("telegram.bot.register.step.account-type.request");
    ...
    protected Void stepEmailProcess(Update update) {
        ...
        if (userExistsByEmail(email)) {
            String text = localizationManager.localize("telegram.bot.validation.email.already-exists", email);
            messageSender.sendMessage(chatId, text);
            prevOrFirstStep(update);
            return null;
        }
    ...
    private Void stepRegionProcess(Update update) {
        ...
        if (!COUNTRIES.containsKey(region)) {
            String text = localizationManager.localize("telegram.bot.validation.region.not-found", region);
            messageSender.sendMessage(chatId, text);
            prevOrFirstStep(update);
            return null;
        }
    ...
    private Void stepPhoneNumberProcess(Update update) {
        ...
        if (userRepository.existsUserByPhoneNumber(UserPhoneNumber.of(phone))) {
            String text = localizationManager.localize("telegram.bot.validation.phone.already-exists", phone);
            messageSender.sendMessage(chatId, text);
            prevOrFirstStep(update);
            return null;
        }
    ...
    private Void stepVendorCountriesProcess(Update update) {
        ...
        if (!COUNTRIES.containsKey(commandOrCountry)) {
            String text = localizationManager.localize("telegram.bot.validation.region.not-found", commandOrCountry);
            messageSender.sendMessage(chatId, text);
            prevOrFirstStep(update);
            return null;
        }
    ...
    private Void stepConfirmPasswordRequest(Update update) {
        long chatId = safeGetChatId(update);
        String text = localizationManager.localize("telegram.bot.register.step.password-confirmation.request");
        messageSender.sendMessage(chatId, text);
        return null;
    }
    ...
    private Void stepConfirmPasswordProcess(Update update) {
        ...
        // Validate
        String notMatchText = localizationManager.localize("telegram.bot.validation.password.not-match");
    ...
    private Void stepVerifyEmailSendMail(Update update) {
        ...
        String requestText = localizationManager.localize("telegram.bot.register.step.email-confirmation.notification", registerObject.request.email);
        messageSender.sendMessage(chatId, requestText);
    ...
    private Void stepVerifyEmailRequest(Update update) {
        ...
        if (registerObject.emailVerificationAttempts <= 0 || (registerObject.emailVerificationBan != null && registerObject.emailVerificationBan.isAfter(LocalDateTime.now()))) {
            ...
            String text = localizationManager.localize("telegram.bot.validation.email-confirmation.no-retries", minutes, seconds);
            messageSender.sendMessage(chatId, text);
            return null;
        }

        String requestText = localizationManager.localize("telegram.bot.register.step.email-confirmation.request");
        messageSender.sendMessage(chatId, requestText);
        return null;
    }
    ...
    private Void stepVerifyEmailProcess(Update update) {
        ...
        String invalidCodeText = localizationManager.localize("telegram.bot.validation.email-confirmation.invalid", registerObject.emailVerificationAttempts);
        ...
        String successText = localizationManager.localize("telegram.bot.register.step.email-confirmation.success");
    ...
    private Void stepSaveDataRequest(Update update) {
        ...
        String markupAgainText = localizationManager.localize("telegram.bot.register.step.save-data.markup.again");
        String markupAgreeText = localizationManager.localize("telegram.bot.register.step.save-data.markup.agree");
    ...
    protected Void stepSaveDataProcess(Update update) {
        ...
        String saveProcessText = localizationManager.localize("telegram.bot.register.step.save-data.process");
        ...
        String rawUserLogin = request.login;
        UserLogin userLogin = UserLogin.of(rawUserLogin);
        String transliterationLogin = TransliterationManager.transliterate(rawUserLogin);

        if (Locale.forLanguageTag("ru").equals(registerObject.locale)) {
            userLogin.setTransliteration(new HstoreTranslationDto(
                    transliterationLogin,
                    rawUserLogin,
                    transliterationLogin
            ));
        } else if (Locale.forLanguageTag("zh").equals(registerObject.locale)) {
            userLogin.setTransliteration(new HstoreTranslationDto(
                    transliterationLogin,
                    transliterationLogin,
                    rawUserLogin
    ...
    public Void cancel(Update update) {
        ...
        String greeting = localizationManager.localize("telegram.bot.register.start", user.getFirstName());
