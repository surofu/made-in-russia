...
@Component
public class GetUserByEmailMapperResultToResponseEntity implements GetUserByEmail.Result.Processor<ResponseEntity<?>> {
    ...
    public ResponseEntity<?> processNotFound(GetUserByEmail.Result.NotFound result) {
        String errorMessage = String.format("User with email '%s' not found", result.getUserEmail());
