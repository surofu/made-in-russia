package com.surofu.madeinrussia.core.service.moderation.operation;
...
public class SetProductApproveStatus {
    Long id;
    ...
    public interface Result {
        ...
        static Result success(Long id, ApproveStatus status) {
            log.info("Successfully set approve status '{}' for product: {}", status.name(), id);
            return Success.of(status);
        }

        static Result saveError(Long id, Exception e) {
            log.error("Error while approving product: {}", id, e);
            return SaveError.INSTANCE;
        }

        static Result notFound(Long id) {
            log.warn("No approving product found for id '{}'", id);
