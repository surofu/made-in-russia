...
import com.surofu.madeinrussia.core.repository.CurrencyRepository;
...
public class RussianCentralBankCurrencyConverterService implements CurrencyConverterService {
    ...
    @Override
    public BigDecimal convert(CurrencyCode from, CurrencyCode to, BigDecimal amount) throws IOException, InterruptedException {
        if (from.equals(to)) {
            return amount;
        }
        ...
        for (Map.Entry<CurrencyCode, Currency> fromEntry : currencies.entrySet()) {
            for (Map.Entry<CurrencyCode, Currency> toEntry : currencies.entrySet()) {
                if (fromEntry.getKey().equals(toEntry.getKey())) {
                    ...
                    cache.put(toEntry.getKey(), fromEntry.getKey(), multiplier);
                }
            }
        }
