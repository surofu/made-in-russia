...
public class TelegramBotLinkAccountHandler {
    ...
    private Void stepEmailProcess(Update update) {
        ...
        if (userOptional.isPresent()) {
            ...
            if (user.getTelegramUserId() != null) {
                messageSender.sendMessage(chatId, "К этому аккаунту уже привязан аккаунт Телеграм", InlineKeyboardBuilder.create()
                        .row()
                        .button("Начать заново", "/link-account")
                        .build());
                return reset(update);
            }
        }
    ...
    protected Void stepPasswordProcess(Update update) {
        ...
        if (userOptional.isEmpty()) {
            messageSender.sendMessage(chatId, "Некорректные данные", InlineKeyboardBuilder.create()
                    .row()
                    .button("Начать заново", "/link-account")
                    .build());
            return reset(update);
        ...
        if (user.getPassword() == null) {
            ...
            messageSender.sendMessage(chatId, "Что-то пошло не так...", InlineKeyboardBuilder.create()
                    .row()
                    .button("Начать заново", "/link-account")
                    .build()
            );
